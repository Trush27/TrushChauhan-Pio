...
namespace Service.Implementations
{
    public class MenuService : IMenuService
    {
        ...
        public async Task<(List<MenuItemViewModel> Items, int TotalItems)> GetItemsByCategoryPaginatedAsync(int categoryId, int page, int pageSize, string searchTerm)
        {
            var result = await _menuRepository.GetItemsByCategoryPaginatedAsync(categoryId, page, pageSize, searchTerm);
            var mappedItems = result.Items.Select(i => _mappingService.MapToViewItemModel(i)).ToList();
            return (mappedItems, result.TotalItems);
        }
