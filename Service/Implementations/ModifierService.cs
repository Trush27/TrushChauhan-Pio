...
namespace Service.Implementations
{
    public class ModifierService : IModifierService
    {
        ...
        public async Task<(List<ModifierViewModel> Modifiers, int TotalItems)> GetModifiersByGroupPaginatedAsync(int modifierGroupId, int page, int pageSize, string searchTerm)
        {
            var result = await _modifierRepository.GetModifiersByGroupPaginatedAsync(modifierGroupId, page, pageSize, searchTerm);
            var mappedModifiers = result.Modifiers.Select(m => _mappingService.MapToViewModifier(m, modifierGroupId)).ToList();
            return (mappedModifiers, result.TotalItems);
        }
